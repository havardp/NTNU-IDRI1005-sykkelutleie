...
class StorageService {
  ...
  getBikeDetails(id, success) {
    connection.query(
      'select chassis_id, B.model, gear, wheel_size, stolen, storage, luggage, description, day_price from Bike B, Product_Type PT where B.model = PT.model and chassis_id = ?',
      [id],
      (error, results) => {
        if (error) return console.error(error);
